...
const scanHistory = ref<
    Array<{ code: string; time: string; success: boolean; target: string }>
>([]);
...
const handleTransfer = () => {
    ...
    form.post(route("master.troli.scan_pindah_store"), {
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: (errors) => {
            const errorMsg =
                errors.qr || errors.nomor_tujuan || "Gagal memproses data.";
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <Card class="lg:col-span-5 border-2 border-slate-200 shadow-sm">
                <CardHeader class="bg-slate-50 py-4">
                    <CardTitle
                        class="text-sm font-bold flex items-center gap-2 text-slate-600"
                    >
                ...
                <CardContent class="p-6 space-y-4">
                    ...
                    <div
                        v-if="form.errors.nomor_tujuan"
                        class="p-2 bg-red-50 border border-red-200 rounded-lg flex items-center gap-2 text-red-600 animate-in fade-in zoom-in"
                    >
                        <IconAlertCircle class="size-4 shrink-0" />
                        <span
                            class="text-[11px] font-bold uppercase leading-tight"
                            >{{ form.errors.nomor_tujuan }}</span
                        >
                    </div>

                    <p
                        class="text-[10px] text-slate-400 text-center font-bold uppercase tracking-widest leading-relaxed"
                    >
                        Silakan salin nomor dari Index, lalu klik tombol biru
                        untuk menempel.
                    </p>
                </CardContent>
            </Card>
            ...
            <Card
                class="lg:col-span-7 border-2 border-blue-600 shadow-xl overflow-hidden"
            >
                <CardContent class="p-0">
                    <div
                        class="py-12 flex flex-col items-center justify-center space-y-6 relative"
                    >
                        <div
                            v-if="!form.nomor_tujuan"
                            class="absolute inset-0 z-30 bg-white/50 backdrop-blur-[1px] flex items-center justify-center"
                        >
                            <Badge variant="secondary" class="shadow-sm"
                                >Isi Nomor Tujuan Dahulu</Badge
                            >
                        </div>
                        ...
                        <div class="w-full max-w-xs space-y-2 text-center">
                            <span
                                class="text-[11px] font-black text-blue-600 uppercase tracking-[0.3em]"
                                >Scan Produk</span
                            >
                            <input
                                ref="qrInput"
                                v-model="form.qr"
                                :disabled="
                                    !form.nomor_tujuan || form.processing
                                "
                                type="text"
                                maxlength="10"
                            ...
                            <p
                                v-if="form.errors.qr"
                                class="text-xs font-bold text-red-600 mt-2 animate-bounce"
                            >
                                {{ form.errors.qr }}
                            </p>
                        </div>
                    ...
                    <div
                        class="bg-slate-50 border-t p-3 flex justify-between items-center"
                    >
                    ...
                    <div
                        class="divide-y border-t bg-white max-h-40 overflow-y-auto"
                    >
                        <div
                            v-for="(log, i) in scanHistory"
                            :key="i"
                            class="p-3 flex justify-between items-center text-xs animate-in slide-in-from-top-2"
                        >
                            <div class="flex items-center gap-2">
                                ...
                                />
                                <span
                                    class="font-mono text-blue-600 font-bold"
                                    >{{ log.target }}</span
                                >
                            </div>
                            <Badge
                        ...
                        <div
                            ...
                            class="p-6 text-center text-slate-300 italic text-xs"
                        >
